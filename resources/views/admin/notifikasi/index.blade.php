...
.notification-item-page {
    display: flex; gap: 15px; padding: 15px; border-bottom: 1px solid var(--border-color);
    cursor: pointer; transition: background 0.2s;
    align-items: center;
}
.notification-item-page:hover { background: var(--hover-bg); }
.notification-item-page.unread { background: rgba(52,152,219,0.05); border-left: 3px solid var(--primary-color); }
...
.notification-message-page { font-size: 13px; color: var(--text-secondary); margin-bottom: 5px; }
.notification-time-page { font-size: 11px; color: var(--text-muted); }
        ...
        if (data.notifications && data.notifications.length > 0) {
            ...
            const html = data.notifications.map(notif => `
                <div class="notification-item-page ${!notif.is_read ? 'unread' : ''}" data-id="${notif.id}">
                    ...
                    <div class="notification-icon-page" onclick="markAsReadAndRedirect(${notif.id}, '${notif.link || '#'}')">
                        ${getIcon(notif.type)}
                    </div>
