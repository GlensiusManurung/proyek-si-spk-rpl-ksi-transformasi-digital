// ========== CHAT ADMIN - FULL VERSION (SAMA DENGAN SUPERADMIN) ==========
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Chat Admin loaded, Auth ID:', window.ChatConfig?.authId);
    ...
    // Update last seen setiap 3 detik
    setInterval(updateLastSeen, 3000);
    
    // Update online status setiap 1 detik
...
function updateOnlineStatus() {
    ...
    fetch('/chat/online-users')
        ...
        .then(data => {
            ...
            console.log('Online IDs dari server:', onlineIds);
            
            // Convert semua ID ke integer biar aman
            const onlineIdsInt = onlineIds.map(id => parseInt(id));
            
            // Update setiap user di sidebar
            document.querySelectorAll('.chat-user').forEach(user => {
                const userId = parseInt(user.dataset.userId);
                const isOnline = onlineIdsInt.includes(userId);
                
                console.log(`User ${user.dataset.userName} (ID: ${userId}) isOnline: ${isOnline}`);
            ...
            // Update header chat untuk user yang sedang aktif
            if (currentUserId) {
                const isCurrentOnline = onlineIdsInt.includes(parseInt(currentUserId));
                console.log(`Current user (ID: ${currentUserId}) isOnline: ${isCurrentOnline}`);
...
function initEmojiPicker() {
    ...
    emojiContainer.style.cssText = 'position:absolute;bottom:55px;left:0;width:320px;max-height:280px;background:white;border-radius:12px;box-shadow:0 8px 24px rgba(0,0,0,0.15);display:none;z-index:9999;overflow:hidden;flex-direction:column;';
...
function initSearch() {
    // Ganti ID dari 'search-contact' menjadi 'contact-search'
    ...
    if (searchInput) {
        ...
        searchInput.addEventListener('input', function() {
            ...
            const contacts = document.querySelectorAll('.chat-user');
            let visibleCount = 0;
            
            contacts.forEach(contact => {
                const name = contact.dataset.userName?.toLowerCase() || '';
                const role = contact.dataset.userRole?.toLowerCase() || '';
                
                if (term === '') {
                    contact.style.display = 'flex';
                    visibleCount++;
                } else if (name.includes(term) || role.includes(term)) {
                    contact.style.display = 'flex';
                    visibleCount++;
                } else {
            ...
            const existingMsg = document.querySelector('.search-no-result');
            
            if (visibleCount === 0 && term !== '') {
                if (!existingMsg) {
                    const noResultMsg = document.createElement('div');
                    noResultMsg.className = 'search-no-result';
                    noResultMsg.innerHTML = `
                        <i class="bi bi-search"></i>
                        <p>Tidak ditemukan kontak untuk "${escapeHtml(term)}"</p>
                    `;
                    contactList?.appendChild(noResultMsg);
                }
            } else {
        ...
        console.log('Search input with id "contact-search" not found!');
    }
}
